...
class AnnotationParser
{
    ...
    protected function parse($class)
    {
        ...
        foreach ($class->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
            if (false !== $method->getDeclaringClass() && $method->getDeclaringClass()->getName() == $class->getName()) {
                ...
                $annotations[$annotator->getName()] = new Annotation($annotator->getClassName(), $annotator->getName(), $annotator->getParameters());
            }
        }
    ...
    /**
     * @param $name
     * @return bool|Annotation|mixed
     */
    public function getAnnotation($name)
    {
        return $this->annotations[$name] ?? false;
    }
}